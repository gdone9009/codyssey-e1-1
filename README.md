...
## 4. 프로젝트 구조
Root(.): Dockerfile, index.html, README.md를 배치하여 어떤 환경에서도 즉시 빌드 및 실행이 가능하도록 '애플리케이션 패키징' 관점에서 구성했습니다.
images/: 문서화의 가시성을 높이기 위한 시각적 증거 자료들을 별도로 분리하여 관리 효율성을 높였습니다.
practice/: 실제 운영 파일에 영향을 주지 않고 자유롭게 실험할 수 있는 '샌드박스(Sandbox)' 환경을 제공하기 위해 구성했습니다.
...
# -p 포트를 맵핑하여 포워딩하고, -v 볼륨을 지정하였습니다.
# 테스트완료된 명령어를 스크립트화하여 반복되는 도커 실행시 오류를 줄이고, 재현성을 확보할 수 있습니다. (스크립트.sh 작성)
...
# 1. 컨테이너 내부 파일 수정 (임시로 문구 변경)
# 컨테이너 내부 환경과 호스트 환경 사이의 '특수문자 이스케이프 충돌' 문제가 가장 까다로웠습니다.

# 문제 상황: docker exec를 통해 HTML 내용을 수정할 때 따옴표와 리다이렉션(>) 기호가 쉘에 의해 잘못 해석되어 파일이 깨지는 현상이 발생했습니다.

# 가설: 쉘(zsh)이 명령어를 먼저 해석하고 도커로 전달하기 때문에, 이중 인용부호(" ") 내부의 기호들이 쉘 특수문자로 오작동했을 것이라 가정했습니다.

# 확인: 작은따옴표(' ')와 큰따옴표(" ")를 섞어 쓰는 방식과 Heredoc(<<EOF) 방식을 대조 실험하며 출력 결과를 cat으로 확인했습니다.

# 조치: 최종적으로 cat <<EOF 방식과 작은따옴표를 활용한 문자열 캡슐화를 적용하여 환경 간 간섭 없이 데이터를 안전하게 전달하는 솔루션을 확보했습니다.
